...
function render(name){
    ...
    d3.text("http://vis.lab.djosix.com:2023/data/spotify_tracks.csv", function(data) {
        ...
        data.forEach((d) => {
            d['artists']          = d['artists'].split(';')[0];
            d['popularity']       = isNaN(+d['popularity'])       ? 0 : +d['popularity'];
            d['duration_ms']      = isNaN(+d['duration_ms'])      ? 0 : +d['duration_ms'];
            d['danceability']     = isNaN(+d['danceability'])     ? 0 : +d['danceability'];
            d['energy']           = isNaN(+d['energy'])           ? 0 : +d['energy'];
            d['key']              = isNaN(+d['key'])              ? 0 : +d['key'];
            d['loudness']         = isNaN(+d['loudness'])         ? 0 : +d['loudness'];
            d['speechiness']      = isNaN(+d['speechiness'])      ? 0 : +d['speechiness'];
            d['acousticness']     = isNaN(+d['acousticness'])     ? 0 : +d['acousticness'];
            d['instrumentalness'] = isNaN(+d['instrumentalness']) ? 0 : +d['instrumentalness'];
            d['liveness']         = isNaN(+d['liveness'])         ? 0 : +d['liveness'];
            d['valence']          = isNaN(+d['valence'])          ? 0 : +d['valence'];
            d['tempo']            = isNaN(+d['tempo'])            ? 0 : +d['tempo'];
        });
        let NewData = data.map(function(obj) {
            return {
                ...
                'acoust.'       : obj.acousticness,
                'instrumental.' : obj.instrumentalness,
        ...
        var selectData = NewData.filter(function(d){ 
            return d.artists == name; 
        })

        console.log("selectData:",selectData, selectData.length);
        ...
        var corr = jz.arr.correlationMatrix(selectData, cols);
        ...
        grid.forEach(function(d){
            if(d.column_x == d.column_y){
                d.correlation = isNaN(d.correlation) ? 1 : d.correlation;
            }
            else{
                d.correlation = isNaN(d.correlation) ? 0 : d.correlation;
            }
        });
...
var selectname = artists[8]

const onColumnClicked = selectname => {
    render(selectname);
    };

d3.select('#artists_menu')
    .call(dropdownMenu, {
    options: artists,
