...
const margin = { top: 20, right: 20, bottom: 30, left: 200 },
    ...
    height = 18799 - margin.top - margin.bottom; // Adjusted height
    // 20000

// Scales and Axes
const x = d3.scaleLinear().range([0, width]);
const y = d3.scaleBand().rangeRound([0, height], 0.03)
                        .paddingInner(0.08); // Adjusted y scale
...
d3.csv("TIMES_WorldUniversityRankings_2024.csv").then(data => {
    allData = data;
    allData.forEach(d => {
        d["scores_overall"]                 = d["scores_overall"]=='n/a'               ? 0 : +d["scores_overall"];
